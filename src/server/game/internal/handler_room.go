...
func (dl *Dealer) getPlayerInfoResp() []*msg.UserInfo {
	...
	for k, v := range playerInfoResp {
		if v != nil && betGod.WinCount == v.WinCount {
			betGod = v
			// 再把其余人按照投注数排序
			playerInfoResp = append(playerInfoResp[:k], playerInfoResp[k+1:]...)
			break
		}
	}
